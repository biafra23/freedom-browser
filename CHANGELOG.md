# Changelog

All notable changes to Freedom will be documented in this file.

## [0.7.1] - 2026-05-06

### Added

- **Verified ENS shield in the address bar.** ENS-resolved navigations (`vitalik.eth`, `bzz://swarm.eth`, `ipfs://docs.ipfs.tech`, etc.) now show a trust shield next to the address bar. Clicking the shield opens a popover with the full resolved URI and per-provider verification details. If the resolution can't reach quorum, the popover surfaces a warning and — when "Block unverified ENS navigation" is enabled (default) — the navigation lands on an interstitial confirmation page rather than proceeding silently. The popover closes automatically if the underlying resolution goes stale (e.g. after navigating away or starting a fresh lookup).
- **Configurable cross-RPC verification.** A new "Cross-RPC verification" section under `freedom://settings` exposes per-lookup quorum parameters: providers per wave (`K`), required byte-identical matches (`M`), per-provider timeout, block anchor (`latest` vs. fixed N-blocks-back), and anchor TTL. Adjacent toggles control whether unverified resolutions are blocked and whether cross-checking is enabled at all.
- **Editable public RPC providers list.** The same settings page lets you edit the list of Ethereum RPCs Freedom uses for ENS cross-checking — add, remove, or reset to defaults. Fewer than three entries disables verification (with a clear status hint). The configured list flows through both forward (name → contenthash) and reverse (address → name) ENS paths.

### Changed

- Updated bundled nodes: Kubo 0.40.1 → 0.41.0. Bee binary stays at 2.7.1 (already latest); the `@ethersphere/bee-js` SDK was bumped 11.1.1 → 12.1.0, which pulls in the upstream axios 1.x security fixes and lets us drop the local axios override.
- Refreshed the rest of the JS dependency tree to latest within current ranges: Electron 41.2.1 → 41.5.0, ESLint 10.2.1 → 10.3.0, `@scure/bip39` 2.0.1 → 2.2.0, `globals` 17.5.0 → 17.6.0, `micro-key-producer` 0.8.5 → 0.8.6, `@babel/preset-env` 7.29.2 → 7.29.3.

### Security

- **Quorum-based ENS resolution.** ENS contenthash and address lookups now query multiple public RPC providers in parallel and only resolve to "verified" if a quorum returns byte-identical answers — anchored to a stable block (`latest - N`) to eliminate head-of-chain divergence as a false negative. If the first wave can't reach quorum, a second wave escalates against fresh providers rather than accepting the unverified result, and anchor corroboration is hardened against single-RPC manipulation. This raises the bar for a malicious or compromised RPC injecting a fake contenthash for an ENS name; the trust state surfaces in the address-bar shield (see Added). While the second wave is in flight, Freedom kicks off a speculative HEAD probe to the local Bee/Kubo gateway for the candidate hash so first-paint latency stays low even on cold-cache lookups. ENS prefetch info logs sanitise URLs before persisting, and the verification interstitial tightens its CSP and drops a stale docs link.
- Pinned the deeply-nested `uuid` transitive (via `@metamask/utils`) to `^14.0.0` through an `npm` override, clearing the GHSA-w5hq-g745-h8pq advisory in `npm audit`. The advisory only affects `uuid.v3()/v5()/v6()` when called with a caller-supplied output buffer, and the only place `@metamask/utils` actually loads `uuid` is its `fs.cjs` test-sandbox helper (which calls `uuid.v4()` and is not reached from anything we import) — so this is precautionary rather than fixing an exploitable path. Vault encrypt/decrypt (the only `@metamask/browser-passworder` consumer) is unchanged; identity tests still pass.
- Navigating to a Swarm address (`bzz://` or ENS-resolved) now keeps the tab's loading spinner running while the Bee node looks up the content, instead of showing Bee's raw `{"code":404,"message":"address not found or incorrect"}` JSON while peers are still syncing. If the lookup times out we land on a friendlier "Content not ready yet" page; if the Bee HTTP API itself is unreachable we route to the existing "Swarm node is not running" error page. When the destination was an ENS name (e.g. `swarm.eth`), the error page and address bar show the original `ens://…` name rather than the resolved hash.
- Swarm pages are now loaded under the `bzz://` URL scheme directly. `window.location.protocol === 'bzz:'`, the hash is the host, and same-origin relative paths Just Work. Sub-resource fetches (including `<video>` Range requests, CSS `@import`, service workers, and `fetch()` calls before any preload-installed wrapper) flow through a main-process protocol handler that proxies to the local Bee gateway with `Swarm-Chunk-Retrieval-Timeout` + redundancy headers, retries transient `5xx` failures with bounded exponential backoff (~50 s total) and a 30 s per-attempt deadline, and streams the response back. **Note for Swarm site authors:** if your site builds absolute URLs from `window.location` (sniffing `protocol === 'http:'`, scraping the `/bzz/<hash>/` prefix from `pathname`, or appending `/bzz/<ref>/` onto `origin`) it will need updating. See "Migrating Swarm sites to the `bzz://` scheme" in the README.
- IPFS and IPNS pages are now loaded under the `ipfs://` / `ipns://` URL schemes directly, mirroring the `bzz://` change above. `window.location.protocol === 'ipfs:'` (or `'ipns:'`), the CID/name is the host, and same-origin relative paths Just Work. Sub-resource fetches flow through a main-process protocol handler that proxies to the local Kubo gateway and follows Kubo's subdomain-gateway redirect internally, so DevTools, `window.location`, storage origin (cookies, localStorage, IndexedDB, service workers), and the address bar all stay on `ipfs://<cid>/` (or `ipfs://<name>/` for ENS-backed sites) instead of bouncing to `http://<cidv1>.ipfs.localhost:8080/`. ENS-backed IPFS/IPNS sites resolve to `ipfs://<name>/` / `ipns://<name>/` for both top-level navigation and sub-resource requests, with the same cross-transport assertion the `bzz` handler enforces (typing `ipfs://name.eth` whose contenthash is Swarm or IPNS surfaces a 404 rather than silently switching). The renderer canonicalises CIDv0 (`Qm...`), CIDv1 base58btc (`z...`, including codecs and multihash codes that use multi-byte varints — eg. dag-json `0x0129`, blake2b-256 `0xb220`), and base58btc IPNS peer IDs to lowercase encodings (CIDv1 base32 `bafy...` for IPFS / IPNS keys, libp2p-key base36 `k51.../k2k4...` for IPNS peer IDs) on input so the case-insensitive lowercase forms round-trip cleanly through Chromium's standard-scheme URL parser; the webview-preload click interceptor listens to both `click` (primary button) and `auxclick` (real middle-click — modern Chromium dispatches `click` only for the primary button) and covers modified-click / `target="_blank"` dispositions so new-tab opens get the raw mixed-case href before Chromium's `setWindowOpenHandler` path lowercases it. The protocol handler additionally rejects already-lowercased CIDv0 / CIDv1 base58btc / base58btc IPNS sub-resource references with an actionable `400` (pointing the site author at the case-insensitive CIDv1 base32 / libp2p-key base36 forms) rather than forwarding a guaranteed-bad reference to Kubo. Both the renderer's address-bar pipeline and the main-process protocol handler recognise the gateway-form `ipfs://<gateway>/ipfs/<cid>/...` shape — gated by an explicit known-public-gateway / loopback host allowlist (`localhost`, `127.0.0.1`, `dweb.link`, `ipfs.io`, `cf-ipfs.com`, `gateway.pinata.cloud`, `nftstorage.link`, `w3s.link`, `4everland.io`, `ipfs.fleek.co`, etc.) so DNSLink hosts that genuinely publish a literal `/ipfs/<text>` path (`ipns://docs.ipfs.tech/ipfs/coverage`) are not mis-rewritten — and rewrite it to the canonical `ipfs://<cid>/...` form. The `/ipns/` namespace additionally accepts DNSLink-shaped names as the embedded ref, so `ipfs://dweb.link/ipns/docs.ipfs.tech/install` rewrites to `ipns://docs.ipfs.tech/install`. **Note for IPFS site authors:** the same `window.location` anti-patterns flagged in the Swarm migration apply — see "Migrating IPFS sites to the `ipfs://` / `ipns://` scheme" in the README.
- ENS-name URLs now display under their resolved transport scheme: `vitalik.eth` shows as `ipfs://vitalik.eth`, `meinhard.eth` as `bzz://meinhard.eth`, and so on. The legacy `ens://` form keeps working for existing bookmarks and in-HTML links. Typed transport schemes (`bzz://name.eth`, `ipfs://name.eth`, `ipns://name.eth`) act as assertions: if the ENS contenthash points at a different transport, the navigation surfaces an error rather than silently switching.
- Swarm pages backed by an ENS name (`bzz://swarm.eth`) now load under `bzz://<name>/` rather than `bzz://<hash>/`. DevTools, `window.location`, storage origin, and same-origin subresource fetches all see the human-readable name; the `bzz` protocol handler resolves ENS hosts on every request via the in-process resolver cache. Cross-transport mismatches (e.g. `bzz://name.eth` whose contenthash is IPFS) return `404` with an explanatory body. Pinning storage to the ENS name keeps state stable across contenthash updates — see the README "Origin model" note.
- ENS links inside web pages must now carry a scheme: `ens://name.eth`, `bzz://name.eth`, `ipfs://name.eth`, or `ipns://name.eth`. Bare `<a href="vitalik.eth">` is a relative URL by HTML/URL spec rules and is not resolved as ENS. Bare names continue to work in the address bar, where input is always absolute.

### Fixed

- Cleaned up three sources of console noise on `npm start`:
  - New tabs opened to a dweb URL (`bzz://`, `ipfs://`, `ipns://`, `ens://`, `rad://`, `freedom://`) no longer log `Error … GUEST_VIEW_MANAGER_CALL: Error: (-3) loading file:///…/home.html` once per tab. The webview was being pointed at `home.html` synchronously and then redirected to the real target ~50 ms later, leaving Chromium to abort the first load (`net::ERR_ABORTED`) and Electron to log the rejected promise. `createTab` now only direct-loads a narrow allowlist (empty/null defaults to `homeUrl`, plus `http(s)`, `about:blank`, and `homeUrl` itself); everything else parks on `about:blank` and is dispatched to `loadTarget`, which routes the schemes it understands and silently drops the rest. As a side effect, hostile inputs like `file:///etc/passwd`, `data:`, or `javascript:` URLs reaching `createTab` from a `setWindowOpenHandler` / `tab:new-with-url` path now stay on `about:blank` instead of becoming a direct webview navigation.
  - Bee no longer logs `[node/multiresolver] resolver on endpoint failed … endpoint=https://cloudflare-eth.com` on every boot. Bee's built-in ENS resolver is unused (Freedom resolves ENS itself before invoking Bee), but configuring an endpoint is mandatory. Switched the placeholder from `cloudflare-eth.com` (deprecated for years, intermittent failures) to `ethereum.publicnode.com`, which is already the lead provider in the renderer's ENS quorum.
  - The `[ens] <kind> cache hit for <name>` line is now logged at `debug` rather than `info`, so the 20+ per page hits no longer dominate the console. Useful for development; leave the renderer at `info` and it disappears.
- The address bar reflects the in-flight target immediately when an ENS link is clicked or typed, instead of staying on the previous page's URL (or empty for new tabs) for the whole resolution roundtrip.
- The protocol icon updates to the right transport (Swarm/IPFS/IPNS) as soon as the scheme is known, rather than waiting until the page finishes loading. This also fixes the IPFS icon never showing for ENS names.
- The tab loading spinner now stays on while ENS resolves a page-link click, instead of being torn down by the phantom abort that follows the custom-protocol intercept.
- Cross-tab UI contamination during ENS resolution: spinner state, modal alert popups, and the resolved URL all stay scoped to the originating tab. Switching to another tab while a background lookup is in flight no longer drops that tab's spinner, surfaces an unrelated alert, or clobbers the foreground tab's address bar.
- Sub-frame load failures (third-party iframes, ad-tech cookie-sync pixels, WalletConnect verify-API attestation iframes, etc.) no longer hijack the main page with the "Content Unavailable" error page. The renderer's `did-fail-load` handler now checks `isMainFrame` and ignores sub-frame failures the way Chromium itself does, so heavy-ad-tech sites (e.g. `https://www.spiegel.de/`) and WalletConnect-using dapps (e.g. `ipfs://ens.eth/`) no longer self-destruct ~2–10 s after loading. The error page itself also uses scheme-aware copy for non-dweb URLs ("Couldn't load this page" / hostname-specific message) instead of unconditionally referencing "the decentralized network", which was misleading for plain HTTPS failures.
- IPFS / IPNS loads on macOS no longer fail with "kubo gateway unreachable" the moment Kubo emits its subdomain-gateway redirect. The protocol handler now uses Electron's `net.fetch` (Chromium's network stack — RFC 6761 `*.localhost` resolution) instead of Node's `globalThis.fetch`, which on macOS routes `*.localhost` through the OS resolver chain and returns `ENOTFOUND`. The handler also gains a per-attempt deadline (30 s) — mirroring the bzz handler's `ATTEMPT_TIMEOUT_MS` — so a stalled Kubo (crashed mid-response, paused worker, debugger breakpoint on the gateway) surfaces a `504` instead of hanging the page load indefinitely.
- CIDv1 base32 references with codecs other than dag-pb / raw — most commonly dag-json (`bagu…`), and any codec whose varint top 2 bits aren't `0b01` — no longer 400 with "invalid ipfs reference". The validation regexes used in the protocol handler, ENS prefetch, and renderer's address-bar pipeline previously hard-coded `baf` as the CIDv1 base32 prefix, which only covers codecs whose varint puts `f` in the 3rd base32 chunk. The relaxed `ba` prefix admits the full range of CIDv1 codecs while still constraining shape (CIDv1 always has `a` as the 2nd base32 char, since the version byte 0x01 contributes a leading `0b00000` chunk). Same fix applied in four places to keep the validations in sync.
- Address-bar / link-click rewrite for ports on the gateway-form `ipfs://localhost:<port>/ipfs/<cid>/...`. Kubo's auto-generated directory listings emit protocol-relative anchors (`<a href="//localhost:8080/ipfs/<cid>">`) that Chromium resolves against the page's `ipfs://` origin to a URL with the port preserved (`ipfs:` has no default port). The renderer's `parseIpfsInput` is byte-level on purpose (to preserve base58btc case), so it now strips a trailing `:<port>` before the gateway-host allowlist check; without that the rewrite was being skipped and the address bar permanently showed the gateway-origin form.
- `<a target="docs" href="ipfs://...">` links now reuse the `docs` named tab the same way `<a target="docs" href="https://...">` does. The webview-preload click interceptor catches every dweb-link click before Chromium can lowercase the host (otherwise the case-sensitive base58btc bytes would be lost), but the previous implementation treated every modified click / `target="_blank"` / named-target click as a generic "newTab" without forwarding the target name. The named-target reuse logic was extracted from `tabs.js` into `openInNewTabWithTarget`, and both the IPC `tab:new-with-url` path and the renderer's `link:navigate` handler now route through it.
- Typed-address ENS navigation now flows through the same `loadTarget` path as in-page link clicks, so the trust shield, interstitial pages, and address-bar transport icon render consistently regardless of whether the URL was typed or clicked. Reverse-resolution (address → name) also respects the configured ENS public RPC providers.

## [0.7.0] - 2026-04-19

### Added

- Experimental Identity & Wallet system (Settings > Experimental):
  - Password-protected vault with auto-lock
  - Touch ID quick-unlock on macOS
  - Multiple wallets and accounts, with Ethereum and Gnosis Chain support
  - Publisher Identities screen
  - Configurable ENS RPC
- dApp connections via injected EIP-1193 `window.ethereum` provider, announced via EIP-6963:
  - Per-origin permission grants with a connection banner and management screen
  - Dedicated approval screens for message signing and transactions, with optional auto-approve
- `ethereum:` URI scheme (EIP-681): links like `<a href="ethereum:vitalik.eth@1?value=1e16">` pre-fill the wallet Send screen (native-asset sends only)
- Swarm publishing from a connected Bee node:
  - `freedom://publish` setup page with readiness checklist and funding actions (chequebook deposit, CowSwap swap-to-xBZZ)
  - Stamp manager with batch list, purchase flow, and extension
  - Publish history
  - Experimental `window.swarm` dApp provider with publish and feed journal APIs, gated by per-origin approval
- Wallet Send accepts ENS names (`.eth`, `.box`, subdomains), and shows the recipient's verified primary ENS name on the review screen
- Bee node can now run in light mode (previously ultra-light only)
- Linux AppImage distribution target

### Changed

- ENS resolution uses the Universal Resolver: 3–4× fewer RPC round-trips on cold-cache `.eth` / `.box` navigation; names normalized per ENSIP-15
- Settings moved from a modal to a full `freedom://settings` page
- Toolbar icons, nodes menu, and experimental settings polished for consistency
- Updated bundled nodes: Bee 2.7.0 → 2.7.1, Kubo 0.39.0 → 0.40.1, Radicle 1.6.1 → 1.8.0 (rad-httpd 0.23.0 → 0.24.0)
- Upgraded Electron to 41; all other dependencies refreshed to latest

### Fixed

- IPFS sites using `_redirects` now resolve correctly

## [0.6.2] - 2026-03-01

### Added

- Experimental support for Radicle (decentralized Git hosting) on macOS and Linux:
  - Enable or disable Radicle from Settings > Experimental
  - `rad://` URL handling across navigation and rewriting
  - Bundled Radicle node lifecycle management and packaging support
  - Integrated repo browser page and GitHub-to-Radicle import bridge
  - Automatic seeding of Freedom's canonical Radicle repository when running the bundled node
- Swarm encrypted reference support in navigation and URL rewriting (including 64- and 128-character hex references)

### Fixed

- `Cmd/Ctrl+L` now reliably focuses the address bar even when web content has focus
- Pressing `Cmd/Ctrl+L` and `Escape` now consistently closes open menus and clears stale focus highlights
- Pinned tabs can no longer be closed through keyboard-accelerator close-tab actions

### Security

- Validate protocol-specific identifiers in IPC handlers and URL rewriting to block malformed or malicious input

## [0.6.1] - 2026-02-08

First public open-source release.

### Added

- Keyboard shortcuts: Ctrl+PgUp/PgDn to switch tabs, Ctrl+Shift+PgUp/PgDn to reorder tabs, Ctrl+F4 to close tab, Ctrl+Shift+T to reopen closed tabs, Ctrl+Shift+B to toggle bookmark bar, F11 for fullscreen, F12 for devtools
- Bookmark bar toggle that persists to settings and always shows on new tab page
- About panel with version, copyright, credits, website, and app icon
- DNS-over-HTTPS resolvers (Cloudflare DoH, eth.limo) for reliable dnsaddr and DNSLink resolution
- ESLint, Prettier, and EditorConfig for consistent code formatting

### Changed

- Split reload into soft (Ctrl+R, uses cache) and hard (Ctrl+Shift+R, bypasses cache); toolbar reload button defaults to soft, Shift+click for hard
- Switch IPFS content discovery from DHT to delegated routing via cid.contact

### Fixed

- Address bar staying focused after selecting autocomplete suggestion
- Unreadable pages in dark mode — inject light background/text defaults for external pages that don't support dark mode
- ENS resolution reliability: replace broken RPC providers (llamarpc, ankr, cloudflare-eth → drpc, blastapi, merkle) and fix failed handle cleanup
- View-source address bar and title not updating correctly
- IPFS routing and DNSLink resolution on networks with broken or slow local DNS

### Security

- Add Content Security Policy headers to all internal HTML pages
- Validate IPFS CID format, IPNS names, and block malformed `bzz://` requests
- Harden webview preferences, restrict `freedomAPI` to internal pages only, tighten local API CORS and IPC base URLs, redact logged URLs
- Resolve all npm audit vulnerabilities (11 total: 10 high, 1 moderate)
- Updated dependencies: Electron 39→40, electron-builder 26.0→26.7, better-sqlite3 12.5→12.6, electron-updater 6.6→6.7

## [0.6.0] - 2026-01-01

First public preview (binary-only).
