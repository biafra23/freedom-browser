...
- IPFS and IPNS pages are now loaded under the `ipfs://` / `ipns://` URL schemes directly, mirroring the `bzz://` change above. `window.location.protocol === 'ipfs:'` (or `'ipns:'`), the CID/name is the host, and same-origin relative paths Just Work. Sub-resource fetches flow through a main-process protocol handler that proxies to the local Kubo gateway and follows Kubo's subdomain-gateway redirect internally, so DevTools, `window.location`, storage origin (cookies, localStorage, IndexedDB, service workers), and the address bar all stay on `ipfs://<cid>/` (or `ipfs://<name>/` for ENS-backed sites) instead of bouncing to `http://<cidv1>.ipfs.localhost:8080/`. ENS-backed IPFS/IPNS sites resolve to `ipfs://<name>/` / `ipns://<name>/` for both top-level navigation and sub-resource requests, with the same cross-transport assertion the `bzz` handler enforces (typing `ipfs://name.eth` whose contenthash is Swarm or IPNS surfaces a 404 rather than silently switching). The renderer canonicalises CIDv0 (`Qm...`) to CIDv1 base32 (`bafy...`) and base58btc IPNS peer IDs to libp2p-key base36 (`k51...` / `k2k4...`) on input so the case-insensitive lowercase encodings round-trip cleanly through Chromium's standard-scheme URL parser; the webview-preload click interceptor covers same-tab clicks **and** modified-click / middle-click / `target="_blank"` dispositions so new-tab opens get the raw mixed-case href before Chromium's `setWindowOpenHandler` path lowercases it. The protocol handler additionally rejects already-lowercased CIDv0 / base58btc IPNS sub-resource references with an actionable `400` (pointing the site author at the case-insensitive CIDv1 base32 / libp2p-key base36 forms) rather than forwarding a guaranteed-bad reference to Kubo. Both the renderer's address-bar pipeline and the main-process protocol handler recognise the gateway-form `ipfs://<gateway>/ipfs/<cid>/...` shape — including public gateways like `dweb.link`, `ipfs.io`, and `cf-ipfs.com`, not just `localhost` — and rewrite it to the canonical `ipfs://<cid>/...` form, gated by a strict embedded-key check so DNSLink sites that publish a literal `/ipfs/<text>` path are not mis-rewritten. **Note for IPFS site authors:** the same `window.location` anti-patterns flagged in the Swarm migration apply — see "Migrating IPFS sites to the `ipfs://` / `ipns://` scheme" in the README.
