...
- Updated the bundled Ant node to v0.5.19; runtime postage-batch management (added in v0.5.8) means publishing to Swarm (buying stamps, uploading data/files/sites) works end-to-end in light mode, and v0.5.19 restores Bee-compatible `/wallet` and stamp-purchase behavior so balance checks and feed/post retrieval work without Ant-specific browser logic
